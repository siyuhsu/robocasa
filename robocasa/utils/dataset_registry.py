...
PRETRAINING_TASKS = dict(
    ...
    # pretrain300 minus navigation tasks.
    # Navigation tasks removed (present in TARGET_TASKS but not TARGET_TASKS_NO_NAVIGATION):
    #   atomic_seen:    NavigateKitchen
    #   composite_seen: DeliverStraw, GetToastedBread, PackIdenticalLunches, ScrubCuttingBoard,
    #                   SearingMeat, SetUpCuttingStation, StackBowlsCabinet, SteamInMicrowave,
    #                   StirVegetables, StoreLeftoversInBowl
    # Tasks below that are NOT in TARGET_TASKS at all are marked with "# TODO: verify"
    # and should be manually inspected.
    pretrain300_no_navigation=[
        # atomic tasks (NavigateKitchen removed)
        "AdjustToasterOvenTemperature",
        "AdjustWaterTemperature",
        "CloseBlenderLid",
        "CloseCabinet",
        ...
        "CheesyBread",
        "LowerHeat",
        # composite tasks (filtered to match composite_demos/)
        "AddIceCubes",
        "AddSugarCubes",
        "AddSweetener",
        "AdjustHeat",
...
# Navigation tasks are excluded here because they require the robot's mobile base
# to traverse long distances to a dining counter / stool area, which makes subtask
# segmentation and VLM labeling fundamentally different from manipulation-only tasks.
#
# Tasks that involve navigation:
#   atomic_seen  : NavigateKitchen  (the task itself is pure navigation)
#   composite_seen: DeliverStraw, GetToastedBread, StoreLeftoversInBowl
#                  (all use DINING_COUNTER / STOOL fixture for cross-room delivery)
#   composite_unseen: ArrangeBreadBasket, GarnishPancake, PortionHotDogs,
#                     RecycleBottlesByType
#                    (all use DINING_COUNTER / STOOL fixture for cross-room delivery)
TARGET_TASKS_NO_NAVIGATION = dict(
    atomic_seen=[
        "CloseBlenderLid",
        "CloseFridge",
        "CloseToasterOvenDoor",
...
TASK_SET_REGISTRY = dict(
    ...
    pretrain300_no_navigation=PRETRAINING_TASKS["pretrain300_no_navigation"],
    ...
    # target tasks with navigation excluded
    atomic_seen_no_nav=TARGET_TASKS_NO_NAVIGATION["atomic_seen"],
    composite_seen_no_nav=TARGET_TASKS_NO_NAVIGATION["composite_seen"],
    composite_unseen_no_nav=TARGET_TASKS_NO_NAVIGATION["composite_unseen"],
    target_no_nav=TARGET_TASKS_NO_NAVIGATION["atomic_seen"]
    + TARGET_TASKS_NO_NAVIGATION["composite_seen"]
    + TARGET_TASKS_NO_NAVIGATION["composite_unseen"],
...
DATASET_SOUP_REGISTRY = dict(
    ...
    # target datasets with navigation excluded
    target_atomic_seen_no_nav=get_ds_soup(
        split="target", task_set="atomic_seen_no_nav", source="human"
    ),
    target_composite_seen_no_nav=get_ds_soup(
        split="target", task_set="composite_seen_no_nav", source="human"
    ),
    target_composite_unseen_no_nav=get_ds_soup(
        split="target", task_set="composite_unseen_no_nav", source="human"
    ),
    target_no_nav=get_ds_soup(
        split="target", task_set="target_no_nav", source="human"
    ),
    ...
    pretrain_human300_no_navigation=get_ds_soup(
        split="pretrain", task_set="pretrain300_no_navigation", source="human"
    ),
