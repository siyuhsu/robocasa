...
class Args:
    ...
    task_set: list = None
    env_names: list = None  # Direct list of env names; overrides task_set if provided
    ...
    zero_base_motion: bool = False  # Zero out base motion dims (action[7:11]) from model output
...
def eval_main(args: Args) -> None:
    ...
    for env_name in all_env_names:
        # try:
        eval_env(env_name, split, log_dir, num_trials, resize_size, replan_steps, host, port, args.seed, args.zero_base_motion)
        # except Exception as e:
        #     print("Exception!")
        #     print(e)

def eval_env(env_name, split, log_dir, num_trials, resize_size, replan_steps, host, port, seed, zero_base_motion=False):
    ...
    print(f"[{t_start.strftime('%Y-%m-%d %H:%M:%S')}] START {env_name}", flush=True)
    ...
    print(f"[{t_end.strftime('%Y-%m-%d %H:%M:%S')}] END   {env_name}  elapsed={elapsed}", flush=True)
