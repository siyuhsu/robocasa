...
from subtask_order import correct_subtask_order
...
def label_one_episode(
    ...
    vp = video_path(suite_dir, ep_idx, video_key)
    ...
    segment_labels_raw = {k: list(v) for k, v in segment_labels.items()}
    segment_labels = correct_subtask_order(segment_labels, overall_segment, subtask_list)
    # subtask = LNDS-merged; reasoning = RAW per-segment (every triple-segment
    # segment keeps its own VLM reasoning, finer than the merged subtask).
    _fa_corr = build_frame_annotations(overall_segment, segment_labels)
    _fa_raw = build_frame_annotations(overall_segment, segment_labels_raw)
    frame_annotations = [
        [_fa_corr[i][0] if i < len(_fa_corr) and len(_fa_corr[i]) >= 1 else "unknown",
         _fa_raw[i][1] if i < len(_fa_raw) and len(_fa_raw[i]) >= 2 else (_fa_corr[i][1] if i < len(_fa_corr) and len(_fa_corr[i]) >= 2 else "unknown")]
        for i in range(len(_fa_corr))]
    ...
    return {
        ...
        "segment_labels_raw": segment_labels_raw,
...
def main():
    ...
    p.add_argument("--episodes", type=int, nargs="*", default=None,
                   help="explicit episode indices (overrides start_ep/num_episodes)")
    ...
    for suite_name, sdir in suite_dirs:
        ...
        total = len(eps)
        if args.episodes:
            ep_indices = [e for e in args.episodes if 0 <= e < total]
        else:
