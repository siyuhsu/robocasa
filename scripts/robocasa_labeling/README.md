...
## Samples
`samples/` — one full-CoT review video per task (24), generated by `unified_cot_viz.py`:
agentview + palette bbox/labels + red-past/cyan-future gripper trajectory, with the
right-side panel (INSTRUCTION / SUBTASK / REASON / LONG PLAN, current step highlighted).
